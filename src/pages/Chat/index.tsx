...
const REPLY_MAP: Record<string, Array<(ctx: ConversationContext) => string>> = {
  ...
  performance: [
    (ctx) => {
      if (ctx.role === 'pfl') {
        return `📊 ${ctx.displayName}，绩效分析报告已生成：\n\n🏆 本季度排名：\n1. 市京津冀协同办 92分 A级 ↑5.2%\n2. 财务处 90分 A级 ↑2.1%\n3. 投资处 87分 B级 → 持平\n4. 高技术处 85分 B级 ↑1.5%\n5. 开放处 83分 B级 ↓0.8%\n6. 人事处 80分 B级 ↑0.5%\n7. 办公室 76分 C级 ↓3.8%\n8. 综合事务中心 74分 C级 ↓1.2%\n\n⚠️ 重点关注：\n• 办公室连续两季度下降，建议启动绩效改进计划\n• 经济信息中心数字化转型指标完成率仅45%\n\n💡 决策建议：\n1. 对C级处室启动绩效面谈\n2. 参考A级处室经验进行经验推广\n3. 建议下季度增设"跨处室协作"加分项\n\n是否需要生成详细的绩效改进建议？`
      }
      return `📊 绩效分析报告已生成：\n\n🏆 本季度排名：\n1. 市京津冀协同办 92分 A级 ↑5.2%\n2. 财务处 90分 A级 ↑2.1%\n3. 投资处 87分 B级 → 持平\n...\n7. 办公室 76分 C级 ↓3.8%\n\n⚠️ 重点关注：\n• 办公室连续两季度下降，建议启动绩效改进计划\n• 经济信息中心数字化转型指标完成率仅45%\n\n是否需要生成详细的绩效改进建议？`
    },
    (ctx) => {
      return `📈 绩效考核情况汇总：\n\n全委平均分：85.3分\nA级处室：2个（市京津冀协同办、财务处）\nB级处室：4个\nC级处室：2个（办公室、综合事务中心）\n\n💡 AI建议：\n1. 办公室建议参考财务处的预算管控经验\n2. 经济信息中心需增加数字化转型资源投入\n3. 建议建立跨处室协作机制提升整体效能\n\n需要我导出完整绩效报告吗？`
    },
    (ctx) => {
      return `📊 绩效趋势分析：\n\n近4个季度变化趋势：\n\n📈 上升处室：\n  • 市京津冀协同办：82→85→89→92（持续上升🏆）\n  • 财务处：86→88→89→90（稳步上升）\n\n📉 下降处室：\n  • 办公室：82→80→79→76（连续3季度下降⚠️）\n  • 综合事务中心：78→77→76→74（缓慢下滑）\n\n➡️ 持平处室：\n  • 投资处：86→87→87→87（稳定但缺乏突破）\n\n🎯 关键洞察：\n1. 市京津冀协同办上升主要得益于"创新项目"加分项\n2. 办公室下降与"服务满意度"指标直接相关\n3. 建议投资处设定突破性目标\n\n需要查看某个部门的详细绩效画像吗？`
    },
  ],
